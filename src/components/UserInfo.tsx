        ...
        <div className="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg">
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex flex-col">
                    ...
                    <div id="name" className="p-3 bg-gray-100 rounded-md">{FAKE_USER.userName}</div>
                </div>

                <div className="flex flex-col">
                    <label htmlFor="email" className="text-gray-700 font-semibold mb-2">Email</label>
                    <div id="email" className="p-3 bg-gray-100 rounded-md">{FAKE_USER.email}</div>
                </div>

                <div className="flex flex-col">
                    <label htmlFor="phone" className="text-gray-700 font-semibold mb-2">Phone Number</label>
                    <div id="phone" className="p-3 bg-gray-100 rounded-md">(+91){FAKE_USER.phone}</div>
                </div>
            ...
            <footer className="flex justify-around mt-8 w-flex-wrap flex-col text-center md:flex-row md:items-center">
                ...
                <Button additionalClasses="w-44 my-4" onClick={handleDirect}>{FAKE_USER.role === 'user' ? 'Your Order History' : 'Manage all users'}</Button>
                <Modal>
                    <Modal.Open opens="form">
                        <Button additionalClasses={`w-44 my-4 ${FAKE_USER.role === 'admin' ? '': 'hidden'}`}>Add more Product</Button>
                    </Modal.Open>
