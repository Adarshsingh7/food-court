	...
	return (
		<div className='py-6 px-4'>
			<h2 className='text-xl font-semibold mb-8'>
				Please fill the details before placing
			</h2>
			<Form method='POST'>
				<div className='mb-5 flex gap-2 flex-col sm:flex-row sm:items-center'>
					...
					<div className='flex flex-col grow'>
						<input
							...
							type='text'
							name='customerName'
							defaultValue={FAKE_USER.userName}
							required
						/>
					</div>
				</div>
				...
				<div className='mb-5 flex gap-2 flex-col sm:flex-row sm:items-center relative'>
					...
					<div className='grow'>
						<input
							...
							type='text'
							name='address'
							defaultValue={FAKE_USER.address}
							required
						/>
					</div>
				</div>
...
export const action = async ({ request }: { request: Request }) => {
	...
	const parsedData = {
		...data,
		orderedItem:
			data.orderedItem instanceof File
				? data.orderedItem.name
				: JSON.parse(data.orderedItem),
	};
	const order = {
		...parsedData,
		status: 'Ordered',
